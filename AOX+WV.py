...
# # Web Page Setup
# st.set_page_config(page_title="WVF Near 0 Scanner", layout="wide")
# st.title("💥 WVF <= 0.05 Filtered - Custom Stocks 5m Scanner")
# st.write("Displays only those stocks where the WVF Value is 0.05 or less (e.g., 0.02, 0.03, etc.)")
...
# with st.spinner('Scanning stocks in real-time...'):
#     try:
#         # Bulk Data Download
#         all_data = yf.download(stocks, period="5d", interval="5m", group_by='ticker', progress=False)
...
#             if len(df) < pd_period + 2:
#                 continue
            
#             # Yesterday's Daily High/Low Logic
#             df_daily = df.copy()
#             df_daily['DateOnly'] = df_daily.index.date
#             unique_dates = sorted(df_daily['DateOnly'].unique())
            
#             if len(unique_dates) < 2:
#                 continue 
                
#             prev_date = unique_dates[-2] 
#             df_prev_day = df_daily[df_daily['DateOnly'] == prev_date]
...
#             highest_close = df['Close'].rolling(window=pd_period).max().iloc[-1]
#             wvf_val = ((highest_close - cur_low) / highest_close) * 100
...
# else:
#     st.info("🎯 No stocks currently have a WVF value below 0.05.")
